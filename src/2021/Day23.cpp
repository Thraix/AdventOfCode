...
namespace y2021::day23
{
  ...
  bool ValidateState(int height, const State& state)
  {
    return Find(state, {2, 1}, 'A') && Find(state, {2, 2}, 'A') && Find(state, {4, 1}, 'B') &&
           Find(state, {4, 2}, 'B') && Find(state, {6, 1}, 'C') && Find(state, {6, 2}, 'C') &&
           Find(state, {8, 1}, 'D') && Find(state, {8, 2}, 'D');
  }

  int Heuristic(int height, const State& state)
  {
    int h = 0;
  ...
  std::vector<std::pair<int, State>> ValidStates(int height, const State& state)
  {
    ...
    for (auto& piece : state)
    {
      ...
      {  // Piece is in the room, move to hallway
        ...
        {
          for (int i = piece.first.y + 1; i < 1 + height; i++)
          {
          ...
          for (int i = piece.first.x - 1; i >= 0; i--)
          {
            ...
            if (i == 0 || i % 2 == 1 || i == 10)
            {
              if (state.count({i, 0}) == 0)
              {
                ...
                newStates.emplace_back(std::pair<int, State>{
                  (piece.first.y + std::abs(piece.first.x - i)) * GetCost(piece.second), newState});
              }
              else
                break;
            }
        ...
        if (canMove)
        {
          ...
          if (pos != 0)
          {
            ...
            newStates.emplace_back(
              std::pair<int, State>{(pos + std::abs(piece.first.x - x)) * GetCost(piece.second), newState});
          }
        }
      }
    }
  ...
  {
    return Helper::AStar(h, state, Heuristic, ValidStates, ValidateState);
  }

  REGISTER_DAY(2021, Day23, State, int);

  REGISTER_TEST_EXAMPLE(2021, Day23, ExampleInput, 1, 12521);
  REGISTER_TEST(2021, Day23, Input, 1, 14350);
  REGISTER_TEST_EXAMPLE(2021, Day23, ExampleInput, 2, 44169);
  REGISTER_TEST(2021, Day23, Input, 2, 49742);
