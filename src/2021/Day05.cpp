...
namespace y2021::day05
{
  ...
  REGISTER_DAY(2021, Day05, std::vector<Line>, int);

  REGISTER_TEST_EXAMPLE(2021, Day05, ExampleInput, 1, 5);
  REGISTER_TEST(2021, Day05, Input, 1, 5442);
  REGISTER_TEST_EXAMPLE(2021, Day05, ExampleInput, 2, 12);
  REGISTER_TEST(2021, Day05, Input, 2, 19571);
  ...
  OUTPUT1(input)
  {
    ...
    int count = 0;
    overlap.Each(
      [&](const Array2D<uint8_t>& overlap, const Index2D& index)
      {
        if (overlap[index] > 1)
          count++;
      });
