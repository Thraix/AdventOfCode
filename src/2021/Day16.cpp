...
namespace y2021::day16
{
  ...
  std::pair<int64_t, int64_t> ReadPacket(const std::string& str, int& offset)
  {
    ...
    else
    {
      ...
      if (type == 0)
        return {versionTotal,
                std::accumulate(values.begin(), values.end(), 0ll, [](int64_t sum, int64_t val) { return sum + val; })};
      else if (type == 1)
        return {
          versionTotal,
          std::accumulate(values.begin(), values.end(), 1ll, [](int64_t prod, int64_t val) { return prod * val; })};
      else if (type == 2)
        return {
          versionTotal,
          std::accumulate(
            values.begin(), values.end(), values.front(), [](int64_t max, int64_t val) { return std::min(max, val); })};
      else if (type == 3)
        return {
          versionTotal,
          std::accumulate(
            values.begin(), values.end(), values.front(), [](int64_t min, int64_t val) { return std::max(min, val); })};
      else if (type == 5)
  ...
  REGISTER_DAY(2021, Day16, std::string, int64_t);

  REGISTER_TEST_EXAMPLE(2021, Day16, ExampleInput, 1, 20);
  REGISTER_TEST(2021, Day16, Input, 1, 984);
  REGISTER_TEST_EXAMPLE(2021, Day16, ExampleInput, 2, 1);
  REGISTER_TEST(2021, Day16, Input, 2, 1015320896946);
